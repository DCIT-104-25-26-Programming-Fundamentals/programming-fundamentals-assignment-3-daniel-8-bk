 ...
 * @param {string} name - Label for the matrix.
 ...
 * @param {number[][]} matrix - Matrix of size M x N.
 * @returns {number[][]} Transposed matrix of size N x M.
 */
...
/**
 * PART B: Adds two matrices of identical size.
...
/**
 * PART C: Multiplies two matrices (M x N) and (N x P).
 * @param {number[][]} matrixA - First matrix (M x N).
 * @param {number[][]} matrixB - Second matrix (N x P).
 * @returns {number[][]} Resulting product matrix (M x P).
 */
...
function main() {
  ...
  console.log('\n=== PART B: Add Two Matrices ===');
  console.log(`Enter a second matrix of size ${rows}x${cols} to add to Matrix A:`);
  ...
  console.log('\n=== PART C: Multiply Two Matrices ===');
  console.log(`For multiplication (A x C), Matrix C must have ${cols} rows.`);
  const colsC = readlineSync.questionInt('Enter number of columns for Matrix C: ');
  const matrixC = readMatrix(cols, colsC, 'Matrix C');
